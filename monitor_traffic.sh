...
# client that needs to be connected over the port :8888. If we do not have a
# connection open from port 8888, kill the server and herewith the docker container.

while true; do
    sleep ${DEFAULT_CONTAINER_RUNTIME:-240}

    if [ `netstat -t | grep -v CLOSE_WAIT | grep ':8888' | wc -l` -lt 2 ]
    then
        pkill jupyter
    fi
